...
@Service
public class ReviewVcountInterceptor implements HandlerInterceptor{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(loginId.equals(reviewDto.getReviewWriter())){
		...
		if(history.contains(reviewNo)) {
			return true;// 저장소에 공지번호가 있으면(읽었다면) 통과
		}
		else{// 저장소에 공지번호가 없다면 번호를 추가하고 갱신
			history.add(reviewNo);
			session.setAttribute("history", history);
		}
		
		// 조회수 증가
		reviewDao.updateReviewReadcount(reviewNo);
