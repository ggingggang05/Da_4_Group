...
import java.util.List;import org.apache.tomcat.websocket.WsRemoteEndpointImplBase;
...
public class AdminMemberController
{
	...
	public String search(@RequestParam(required = false) String column, @RequestParam(required = false) String keyword,
			Model model)
	{
	...
	public String add(@RequestParam String memberId, @RequestParam String blockReason)
	{
		// 차단할 사용자의 Dto를 찾아서
	...
	public String cancel(@RequestParam String memberId, @RequestParam String cancelReason)
	{
	...
	@RequestMapping("/block/searchList")
	public String searchList(@RequestParam(required = false) String memberId, Model model)
	{
		boolean isSearch = memberId != null;
		List<BlockMemberVO> list;
		if(isSearch) {
			list = memberBlockDao.selectBlockListById(memberId);
		} else {
			list = memberBlockDao.selectBlockListById();
		}
		model.addAttribute("list", list);
		return "/WEB-INF/views/admin/member/block/searchList.jsp";
	}
}
